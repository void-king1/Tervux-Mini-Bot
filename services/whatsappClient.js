...
sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const m of messages) {
        ...
        const configOwner =
            config.ownerNumber?.replace(/\D/g, "") || "";

        const senderNumber =
            senderJid?.split("@")[0]?.split(":")[0] || "";

        // Sudo list
        const sudoNumbers = Array.isArray(config.sudoNumbers)
            ? config.sudoNumbers.map(n => String(n).replace(/\D/g, ""))
            : [];
        ...
        // Owner + sudo are allowed
        const isAuthorized = isOwner || isSudo;

        // Completely ignore everyone else.
        // NO ACCESS DENIED MESSAGE IN THE GROUP.
        if (!isAuthorized) continue;

        const isGroup = m.key.remoteJid?.endsWith("@g.us");

        // Where the bot should reply:
        // Group command  -> sender's private chat
        // Private command -> normal private chat
        ...
        if (commandName === "addsudo") {
            // Only the owner can add sudo
            if (!isOwner) {
                return;
            }
            ...
            if (!number) {
                await sock.sendMessage(replyJid, {
                    text: `❌ Usage:\n${prefix}addsudo 234XXXXXXXXXX`
                });
                continue;
            }

            const currentSudos = Array.isArray(config.sudoNumbers)
                ? config.sudoNumbers.map(n => String(n).replace(/\D/g, ""))
                : [];

            if (currentSudos.includes(number)) {
                await sock.sendMessage(replyJid, {
                    text: `⚠️ ${number} is already a sudo.`
                });
                continue;
            }

            currentSudos.push(number);

            updateConfig({
                sudoNumbers: currentSudos
            });
            ...
            await sock.sendMessage(replyJid, {
                text:
                    `✅ *SUDO ADDED*\n\n` +
                    `📱 Number: ${number}\n` +
                    `👑 This number can now use Tervux commands.`
            });
        ...
        if (commandName === "delsudo") {
            // Only the owner can remove sudo
            if (!isOwner) {
                return;
            }
            ...
            if (!number) {
                await sock.sendMessage(replyJid, {
                    text: `❌ Usage:\n${prefix}delsudo 234XXXXXXXXXX`
                });
                continue;
            }

            if (number === configOwner || number === botNumber) {
                await sock.sendMessage(replyJid, {
                    text: "❌ You cannot remove the bot owner."
                });
                continue;
            }
            ...
            const newSudos = currentSudos.filter(n => n !== number);

            updateConfig({
                sudoNumbers: newSudos
            });

            invalidateConfigCache();

            await sock.sendMessage(replyJid, {
                text: `✅ Removed ${number} from the sudo list.`
            });
        ...
        if (commandName === "sudolist") {
            ...
            if (currentSudos.length === 0) {
                await sock.sendMessage(replyJid, {
                    text: "👑 *SUDO LIST*\n\nNo sudo numbers have been added."
                });
            } else {
                const list = currentSudos
        ...
        if (commandName && commands[commandName]) {
            try {
                console.log(
                    `⚡ Executing command: ${commandName} for ${senderNumber}`
                );

                /*
                 * When the command came from a group,
                 * redirect ALL sock.sendMessage() calls
                 * to the sender's private chat.
                 *
                 * This also handles commands that send
                 * media directly instead of returning it.
                 */
                const commandSock = isGroup
                    ? new Proxy(sock, {
                        get(target, property, receiver) {

                            if (property === "sendMessage") {
                                return async (_jid, content, options = {}) => {

                                    // A group message cannot safely be quoted
                                    // inside a private-chat response.
                ...
                /*
                 * Some commands return text/media instead
                 * of sending it themselves.
                 */
                if (result) {
                    const footer =
                        `\n\n━━━━━━━━━━━━━━━━━━━━\n` +
                        `💠 *ℙ𝕠𝕨𝕖𝕣𝕖𝕕 𝕓𝕪 𝕋𝕖𝕣𝕍𝕌𝕏 𝕄𝕚𝕟𝕚-𝔹𝕠𝕥*\n` +
                        `🔗 github.com/JonniTech/Tervux-Mini-Bot`;
